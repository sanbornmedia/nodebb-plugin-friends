...
(function(routes) {
	...
	routes.init = function(params, callback) {
		...
		router.get('/admin/plugins/connection-settings', middleware.admin.buildHeader, controllers.renderConnectionSettings);
		router.get('/api/admin/plugins/connection-settings', controllers.renderConnectionSettings);

		helpers.setupPageRoute(router, '/user/:userslug/friends', middleware, middlewares, controllers.getFriends);
