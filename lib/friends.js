...
friends.requestFriendship = function(uid, toUid, callback) {
	...
	async.parallel({
		...
		profileNotCompleted: async.apply(isProfileCompleted, uid)
		...
		} else if (results.profileNotCompleted) {
			return callback(new Error('[[error:profile-not-completed'));
		}
...
friends.areFriendsOrRequested = function(uid, uids, callback) {
	async.parallel({
		...
		profileIncomplete: async.apply(isProfileCompleted, uid),
	}, function(err, results) {
		if (err) {
			return callback(err);
		}
...
function isProfileCompleted (uid, callback) {
	async.waterfall([
		function (next) {
			getConnectionSettings(next);
		},
		function (requiredFields, next) {
			checkUserData(uid, requiredFields, next)
		}
	], function (err, res) {
		callback(err, res);
	});
...
function getConnectionSettings(callback) {
	meta.settings.get('connection', function(err, settings) {
		if (err) console.log('>>> err fetching connection settings', err);
		var counter = 0;
		var requiredFields = [];
		for (const key of Object.keys(settings)) {
			counter ++;

			if (settings[key] == 'on') requiredFields.push(key);

			if (counter == Object.keys(settings).length) {
				callback(null, requiredFields);
			}
		}
	});
}

function checkUserData (uid, fields, callback) {
	user.getUserFields(uid, fields, function(err, data) {
		if (err) console.log('>>> err fetching user fields', err);
		var completedItems = [];
		var counter = 0;
		for (const key of Object.keys(data)) {
			counter++;
			completedItems.push(data[key] != '');
			if (counter == Object.keys(data).length) {
				if (fields.length == 0) {
					callback(null, false);
				} else {
					callback(null, completedItems.includes(false));
				}
			}
		}
	});
